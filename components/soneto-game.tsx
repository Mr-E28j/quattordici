...
// Función simplificada para dividir palabras en sílabas
const divideWordIntoSyllables = (word: string): string[] => {
  ...
  while (i < normalizedWord.length) {
    ...
    // Si es una vocal y no estamos al final de la palabra
    if (vowels.includes(normalizedWord[i]) && i < normalizedWord.length - 1) {
      // Caso 1: Siguiente carácter es una vocal (posible diptongo o hiato)
      if (vowels.includes(normalizedWord[i + 1])) {
      ...
      // Caso 2: Siguiente carácter es una consonante
      else {
        // Reglas básicas de separación consonántica
        if (
          i + 2 < normalizedWord.length &&
          !vowels.includes(normalizedWord[i + 1]) &&
          vowels.includes(normalizedWord[i + 2])
        ) {
          // Consonante entre vocales va con la segunda vocal
          syllables.push(currentSyllable)
          currentSyllable = ""
        } else if (
          i + 2 < normalizedWord.length &&
          ...
          // Dos consonantes: la primera va con la vocal anterior, la segunda con la siguiente
          if (!isInseparableGroup(normalizedWord[i + 1], normalizedWord[i + 2])) {
            ...
            // Grupo consonántico inseparable va con la siguiente vocal
            syllables.push(currentSyllable)
            currentSyllable = ""
          }
        }
...
// Implementación completamente nueva y simplificada de countMetricalSyllables
...
export default function SonetoGame() {
  ...
  const insertAsterisk = (verseIndex: number, position: number) => {
    ...
    // Encontrar la posición exacta para insertar el asterisco
    // Buscar la posición entre dos vocales
    const vowels = "aeiouáéíóúü"
    let insertPosition = -1

    // Buscar la posición aproximada donde insertar el asterisco
    for (let i = Math.max(0, position - 3); i < Math.min(verse.length - 1, position + 3); i++) {
      if (vowels.includes(verse[i].toLowerCase()) && vowels.includes(verse[i + 1].toLowerCase())) {
        insertPosition = i + 1
        break
      }
    }
    ...
    <div className="bg-card rounded-lg shadow-md p-6 dark:shadow-lg">
                    ...
                    <div className="mt-1 ml-8 text-xs text-muted-foreground">
                      ...
                      {syllableDivisions[index] && syllableDivisions[index].length > 0 && (
                        <div className="mt-2 text-xs">
                          ...
                          {sinalefaPositions[index].length > 0 && (
                            <p className="text-xs mt-1 text-muted-foreground">
                              Las sílabas en azul indican sinalefas (se cuentan como una sola sílaba métrica).
                            </p>
                          )}
                        </div>
                      )}
      ...
      {showTips && (
        <div className="mt-6 p-4 bg-muted rounded-lg">
          ...
          <p className="text-sm text-muted-foreground mt-2">
            Puedes insertar un asterisco (*) entre dos vocales para forzar una sinalefa y reducir el conteo de sílabas.
          </p>
        </div>
      )}
